...
export const getAllCars = async (): Promise<ICar[]> => {
    let cars: ICar[] = []
    cars = await axios.get(`${serverUrl}/garage`)
    .then((response) => {
...
export const engineMode = async(id: number, status: string) => {
    let result;
    try{
        result = (await axios.patch(`${serverUrl}/engine`, null, {params: {id : id, status: status}})).data
    }catch(error){
...
export const createWinner = async(data: IWinner) => {
    console.log(await axios.post(`${serverUrl}/winners`, data, {
        headers: {
            'Content-Type': 'application/json'
        }
    }))
...
export const updateWinner = async(data: IWinner) => {
    await axios.put(`${serverUrl}/winners/${data.id}`,{wins: data.wins , time: data.time},{
        headers: {
            'Content-Type': 'application/json'
        }
    })
