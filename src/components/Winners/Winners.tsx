...
const WinnersPage: FC<IWinnersPageProps> = ({
  show,
  setShow,
  winners,
  cars,
  ...
  useEffect(() => {
    if (winners) {
      const sliced = winnersTable?.slice(
        (currentPage - 1) * 10,
        (currentPage - 1) * 10 + 10,
      );
  ...
  const makeSort = () => {
    ...
    (async function () {
      const sorted = await sortWinners({
        sort: sortParams[0],
        order: sortParams[1],
      });
  ...
  return (
    <>
      <div
        ...
        className="container winners-page"
      >
        ...
        <table className="winners-table">
          ...
          <tbody>
            {paginatedWinners.map((el) => {
              return (
                <tr className="winners-table-row" key={el.id}>
                  <td>{el.id}</td>
                  <td>
        ...
        <div className="pagination-panel">
          ...
          <button
            onClick={() => setCurrentPage(currentPage + 1)}
            disabled={
              paginatedWinners.length < 10 ||
              currentPage * 10 === winners.length
            }
          >
            Next
          </button>
