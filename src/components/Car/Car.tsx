...
import { moveCarById, makeDriveButtonId, makeStopButtonId, calcDuration, checkDriveById, setMoveAnimation } from "../../service/localService";
import { engineMode, removeCar, removeWinner, getCarById } from "../../service/service";
...
const Car: React.FC<ICarProps> = ({color, winnersList,fetchWinners, fetchCars, setCarToEdit, setShowEditModal, raceOn, id}) => {
  ...
  return(
          <>
            ...
            <div className="car-buttons">
              ...
              <button id={makeDriveButtonId(id)} className={`${(raceOn || isMoving)&& "disabled-button"}`} disabled={raceOn || isMoving} onClick={() => handleOnDriveButtonClick(id)}>D</button>
              <button id={makeStopButtonId(id)} className={`${!raceOn && !isMoving  && "disabled-button"}`} disabled={!raceOn && !isMoving} onClick={() => handleOnStopButtonClick(id)}>S</button>
            </div>
          </>
  )
